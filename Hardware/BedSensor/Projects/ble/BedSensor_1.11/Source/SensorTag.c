/**************************************************************************************************
  ...
  Modifications Copyright 2013 Soft Edge Technologies. All rights reserved.
...
// VB
// V1.11 Changed 1000->3000
#define BED_DEFAULT_PERIOD                    3000
...
// V1.1 80->1600 i.e 2 seconds
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL     1600

// Maximum connection interval (units of 1.25ms, 800=1000ms) if automatic parameter update request is enabled
// V1.1 800->2400
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL     2400
...
// GAP GATT Attributes
static uint8 attDeviceName[] = "Nocturne Bed Sensor";
...
// VB
static uint16 sensorBedPeriod = BED_DEFAULT_PERIOD;
...
// VB
static bedCBs_t sensorTag_BedCBs =
{
  bedChangeCB,             // Charactersitic value change callback
};
...
uint16 SensorTag_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & ST_GYROSCOPE_SENSOR_EVT )
  {
    if(gyroEnabled)
    {
      ...
      osal_start_timerEx( sensorTag_TaskID, ST_GYROSCOPE_SENSOR_EVT, GYRO_DEFAULT_PERIOD );
    }
    else
    {
      HalGyroTurnOff();
  ...
  if ( events & ST_BED_SENSOR_EVT )
  {
    ...
    {
      bedEnabled = TRUE;
      resetCharacteristicValue( BED_SERV_UUID, BED_DATA, 0, BED_DATA_LEN);
      osal_start_timerEx( sensorTag_TaskID, ST_BED_SENSOR_EVT, sensorBedPeriod );
    }

    return (events ^ ST_BED_SENSOR_EVT);
  }
...
static void readBedData( void )
{
  uint8 tData[BED_DATA_LEN];

  if (HalBedRead(tData))
  {
    Bed_SetParameter( BED_DATA, BED_DATA_LEN, tData);
  }
}
...
static void resetCharacteristicValues( void )
{
  ...
  // VB
  resetCharacteristicValue( BED_SERV_UUID, BED_DATA, 0, BED_DATA_LEN);
  //resetCharacteristicValue( BED_SERV_UUID, BED_CONF, ST_CFG_SENSOR_DISABLE, sizeof( uint8 ));
}
