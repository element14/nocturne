...
public enum Sensor {
  ...
  ACCELEROMETER(UUID_ACC_SERV, UUID_ACC_DATA, UUID_ACC_CONF) {
    ...
    public void onCharacteristicChanged(final BluetoothGattCharacteristic c) {
       ...
       * arbitrarily defined the positive y direction. (illustrated by the apps
       * accelerometer image)
       */
  ...
  SIMPLE_KEYS(UUID_KEY_SERV, UUID_KEY_DATA, null, null) {
    ...
    public void onCharacteristicChanged(BluetoothGattCharacteristic c) {
      /*
       * The key state is encoded into 1 unsigned byte. bit 0 designates the
       * right key. bit 1 designates the left key. bit 2 designates the side
       * key.
       * 
  ...
  /**
   * Our Model in our MVC-structured code, the sensors update the model when new
   * data arrives. The sensors are therefore the Controllers in our MVC.
   * 
