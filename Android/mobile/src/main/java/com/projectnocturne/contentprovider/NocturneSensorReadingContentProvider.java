...
public final class NocturneSensorReadingContentProvider extends ContentProvider {
	...
	static {
		...
		uriMatcher.addURI(AUTHORITY, SensorReadingDb.DATABASE_TABLE_NAME, ALL_ROWS);
		uriMatcher.addURI(AUTHORITY, SensorReadingDb.DATABASE_TABLE_NAME + "/#", SINGLE_ROW);
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArguments) {
		...
		int deleteCount = database.delete(SensorReadingDb.DATABASE_TABLE_NAME, selection, selectionArguments);
		// notify observers
	...
	public String getType(Uri uri) {
		...
		case SINGLE_ROW:
			// single item
			return "vnd.android.cursor.item/vnd." + AUTHORITY + "." + SensorReadingDb.DATABASE_TABLE_NAME;
		case ALL_ROWS:
			// all items
			return "vnd.android.cursor.dir/vnd." + AUTHORITY + "." + SensorReadingDb.DATABASE_TABLE_NAME;
		default:
	...
	public Uri insert(Uri uri, ContentValues contentValues) {
		...
		long id = database.insert(SensorReadingDb.DATABASE_TABLE_NAME, nullColumnHack, contentValues);
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArguments, String sortOrder) {
		...
		queryBuilder.setTables(SensorReadingDb.DATABASE_TABLE_NAME);

		switch (uriMatcher.match(uri)) {
		case SINGLE_ROW:
			// for single row given ID
			String rowID = uri.getPathSegments().get(1);
			queryBuilder.appendWhere(SensorReadingDb._ID + "=" + rowID);
			break;
		default:
			break;
		}
	...
	public int update(Uri uri, ContentValues contentValues, String selection, String[] selectionArguments) {
		...
		case SINGLE_ROW:
			String rowID = uri.getPathSegments().get(1);
			selection = SensorReadingDb._ID + "=" + rowID
					+ (!TextUtils.isEmpty(selection) ? (" AND (" + selection + ")") : "");
		default:
			break;
		}

		int numberRowsUpdated = database.update(SensorReadingDb.DATABASE_TABLE_NAME, contentValues, selection,
				selectionArguments);
		// notify observers of change in data set
		getContext().getContentResolver().notifyChange(uri, null);
